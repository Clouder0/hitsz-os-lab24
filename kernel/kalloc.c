...
kalloc_cpu(int cpu_id)
{
  ...
  {
    // steal!
    int max_free, max_free_id;
    if(cpu_id > 0) {
      acquire(&kmem[0].lock);
      max_free = kmem[0].freenum, max_free_id = 0;
      release(&kmem[0].lock);
    } else {
      acquire(&kmem[1].lock);
      max_free = kmem[1].freenum, max_free_id = 1;
      release(&kmem[1].lock);
    }
    for (int i = max_free_id + 1; i < NCPU; ++i)
    {
    ...
    acquire(&kmem[s].lock);
    acquire(&kmem[b].lock);
    int steal_num = (kmem[max_free_id].freenum + 1) / 2;
